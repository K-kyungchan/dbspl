...
****알아는 두자(못외워도 찾아서 사용가능)
    모델링 툴을 사용하여 설계를 하게되면 툴에서 설계된 테이블을 
    생성하는 구문을 자동으로 만들어준다.
테이블을 생성하는 문법      
CREATE TABLE 오라클사용자 이름이 들어갈수도 안들어 갈수도 . 테이블명 (컬럼명 컬럼의데이터타입.
                                                                 컬럼명2 컬럼의 데이터타입2...);
테이블생성
테이블명 - rnager 
컬럼은 ranger_no NUMBER 
      ranger_nm VARCHRA2(50),
      reg_dt DATE
...
2. 문자 : VARCHAR2(사이즈- byte), CHAR(X)
         ...
         CARCHAR2(2000) : 최대 2000byte를 담을수 있는 문자타입 
...
3. 날짜 : DATE -7byte 고정 
         일자, 시간(시,분,초) 정보 저장
       VARCHAR2 날짜관리 : YYYYMMDD ==> '20200918' ==> 8바이트 
...
CREATE TABLE dept_test(
             ...
             danme VARCHAR2(14),
             loc VARCHAR2(13));
PRIMARY KEY : UNIQUEW(중복된 값이 발생 할 수없다)
...
만약 dept_test테이블에서 10번 부서를 삭제하게 되면??
...
90번 부소에 속한 9000번 사원의 데이터도 같이 삭제가 된다.
입력시 : 부모(dept) ==> 자식(emp)
삭제시 : 자식(emp) ==> 부모(dept)
...
INSERT INTO emp_test VALUES (9000, 'brown', -5, 'M');                SAL체크
INSERT INTO emp_test VALUES (9000, 'brown', 100, 'T');               성별체크
INSERT INTO emp_test VALUES (9000, 'brown', 100, 'M'); 
...
2. 기존에 존재하는 컬럼의 변경(이름, 데이터 타입)
   ** 데이터타입의 경우는 이미 데이터가 존재하면 수정이 불가능 하다고 보면 됨.
...
*** 데이터 타입을 바꾸는 것은 데이터가 없을 때는 상관 없지만 데이터가 있을 경우는 사이즈를 늘리는것을 제외하고는 불가능 ***

컬럼명 변경 
hp 컬럼을 PHONE으로 컬럼명을 변경 데이터 타입과 다르게 이름의 변경은 자유롭다 
