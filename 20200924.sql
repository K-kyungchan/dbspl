...
행을 잘라서 서로 다른 테이블에 분산 : 개별 테이블의 건수가 줄어드므로 table full access 속도가 빨라진다 

실적 테이블 : 20190101~20191231 실적데이터 ==> SALES_2019 테이블에 저장
             20190101~20191231 실적데이터 ==> SALES_2020 테이블에 저장 
...
일실적 : 한달에 생성 데이터가 4~5000만건 ==> 월단위 파티션 구성 
...
emp 테이블을 이용하여 emp 테이블 존재하고 emp_test에는 없는 사원에 대해서는 신규로 
...
------총이랑 계 따로따로-----
--실습 GROUP_AD2-1 ppt26 --문자열 타입// 숫자 타입에 따라 결과를 결정할수 있다.
SELECT DECODE (GROUPING(job), 1, '총', job) plusjob,
       DECODE (GROUPING(job) || GROUPING(deptno), '01', '소계','11' ,'계',deptno) deptno,
       SUM(sal + NVL(comm,0)) sal
FROM emp
GROUP BY rollup (job,deptno);
...
SELECT CASE
        ...
        CASE
            ...
            ELSE TO_CHAR(deptno) --숫자로 인식하므로 문자로 변환
        END deptno,
        SUM(sal + NVL(comm,0)) sal
FROM emp
GROUP BY rollup (job,deptno);
...
SELECT NVL(dname, '총합') ,job, sum(emp.sal + nvl(comm, 0)) sal
FROM emp , dept 
WHERE emp.deptno  = dept.deptno
GROUP BY ROLLUP (dname, job)
